...
public:
    ...
    inline I constexpr Map(I a) const
    {
        return table[O::template MidBits<P, N>(a)];
    }

    template <typename O, int P>
    inline I constexpr TopMap(I a) const
    {
        static_assert(P + N == O::SIZE, "TopMap inconsistency");
...
public:
    ...
    template <I (*F)(const I&)>
    inline void Build(I a)
    {
        trans.template Build<F>(Num<N>(), a);
    }
};
...
public:
    ...
    inline void Build(I a)
    {
        I n = trans.template Build<F>(Num<N>(), a);
        rec.template Build<F>(F(n));
    }
};
