...
CTxDestination ParentDecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    // Note this will be false if it is a valid Bech32 address for a different network
    bool is_bech32 = (ToLower(str.substr(0, params.ParentBech32HRP().size())) == params.ParentBech32HRP());
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        // Bech32 decoding
        if (dec.hrp != params.ParentBech32HRP()) {
            error_str = strprintf("Invalid or unsupported prefix for Segwit (Bech32) address (expected %s, got %s).", params.ParentBech32HRP(), dec.hrp);
            return CNoDestination();
        }
        int version = dec.data[0]; // The first 5 bit symbol is the witness version (0-16)
        if (version == 0 && dec.encoding != bech32::Encoding::BECH32) {
...
CTxDestination ParentDecodeDestination(const std::string& str, std::string& error_msg, std::vector<int>* error_locations)
{
    return ParentDecodeDestination(str, Params(), error_msg, error_locations);
}

CTxDestination ParentDecodeDestination(const std::string& str)
{
    std::string error_msg;
    return ParentDecodeDestination(str, error_msg);
}
